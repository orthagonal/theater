...
class InventoryItem {
  constructor(item) {
    ...
    this.useImage = document.createElement('img');
    this.useImage.src = '/images/use.jpg';
    this.item = item;
    this.position = { x: iconsWidth / 2, y: 10, radians: 0.35 };
    this.element = document.createElement('img');
    this.width = 0; this.height = 0;
    this.exitPositionX = this.position.x - 10;
    this.exitPositionY = this.position.y - 5;
    this.usePositionX = this.position.x + this.width - 10;
  ...
  draw(context) {
    ...
    if (!this.selected) {
      ...
      context.drawImage(xImage, this.exitPositionX, this.exitPositionY, iconsWidth, iconsHeight);
      // draw the use button:
      context.drawImage(this.useImage, this.usePositionX, this.exitPositionY, iconsWidth, iconsHeight);
    }
    context.restore();
  }
  ...
  hitExit(x, y) {
    return ((x > this.exitPositionX) && (x < this.exitPositionX + iconsWidth)
    && (y > this.exitPositionY) && y < this.exitPositionY + iconsHeight)
  }
  hitUse(x, y) {
    return ((x > this.usePositionX) && (x < this.usePositionX + iconsWidth)
    && (y > this.exitPositionY) && y < this.exitPositionY + iconsHeight)
  }
  handleClick(x, y) {
    if (this.hitExit(x, y)) {
