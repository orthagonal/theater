...
exports.ClientManager = function(gameStream){
	...
	this.restoreOrStart = function(userId, sessionId, callback){
		...
		if (!result){
			...
			self.refreshRoom(object);
			callback(object);
		}
	}

	// restore the room's data based no it having only a name
	this.refreshRoom = function(result)	{
		result.room = exports.RoomGraph[result.roomName]
	}
